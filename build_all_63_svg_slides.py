...
def generate_svg_for_slide(num, title):
  ...
  <g transform="translate(630, 5)">
    ...
    elif 'SCORECARD' in title_upper or 'WRAP-UP' in title_upper or 'READINESS' in title_upper or '5 GATES' in title_upper or '5-GATE' in title_upper:
        return '''<svg viewBox="0 0 800 100" class="slide-svg">
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Slide 6: Universal Environment Setup & Command Blocks with Copy Buttons */
    .setup-slide-grid {
      display: grid;
      grid-template-columns: minmax(0, 0.86fr) minmax(0, 1.54fr);
      gap: 1.00rem;
      height: auto;
      align-items: start;
    }
  ...
  <script>
    ...
    function renderSlide(idx) {
              ...
              <div class="slide-1-instructor-info">
                ...
                <div class="slide-1-social-links">
                  <div class="slide-1-social-pill">
                    ...
                    <span class="social-label">Substack:</span>
                    <a href="https://kenhuangus.substack.com/" target="_blank" rel="noopener noreferrer" class="slide-1-social-link" title="Read Ken Huang's Substack">kenhuangus.substack.com <span class="ext-arrow">↗</span></a>
                  </div>
                  <div class="slide-1-social-pill">
                    <span class="social-icon">💼</span>
                    <span class="social-label">LinkedIn:</span>
                    <a href="https://www.linkedin.com/in/kenhuang8/" target="_blank" rel="noopener noreferrer" class="slide-1-social-link" title="Connect with Ken Huang on LinkedIn">linkedin.com/in/kenhuang8 <span class="ext-arrow">↗</span></a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="setup-left-col">
              <div class="failure-mode-card">
                <div class="failure-mode-header">
                  <span>📚 Environment Resources &amp; Execution Guidance</span>
                </div>
                <div class="failure-mode-body">
                  <div class="setup-links-box" style="border-top:none; padding-top:0;">
                    <div>
      ...
      if (!isCode && !isSkill) {
        ...
        let maxScale = 3.25;
        const isSetupSlide = !!wrapper.querySelector('.setup-slide-grid');
        if (isSetupSlide) {
          maxScale = 1.70;
        } else if (wrapper.querySelector('.sub-bullets') || wrapper.querySelector('.capstone-layout-grid') || wrapper.querySelector('.course-map-columns-grid') || wrapper.querySelector('.thesis-grid') || wrapper.querySelector('.slide-1-container')) {
          maxScale = 1.20;
        } else if (wrapper.querySelector('.main-bullets')) {
          maxScale = 1.50;
        }
        ...
        // Setup slide uses auto-height cards; do not shrink just because the wrapper fills the card.
        let shrinkIter = 0;
        while ((bodyEl.scrollHeight > clientH || (!isSetupSlide && wrapper.offsetHeight > (clientH - 6))) && scale > 0.50 && shrinkIter < 60) {
          scale -= 0.02;
          bodyEl.style.setProperty('--fit-scale', scale.toFixed(2));
          shrinkIter++;
        }
