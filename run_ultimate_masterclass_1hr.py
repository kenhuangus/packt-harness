"""
Comprehensive 1-Hour+ (75+ Min) Masterclass Audio Generator.
Zero repetitive phrasing across all 86 slides.
...
# Exhaustive, rich technical lectures for all 86 slides (~9,200+ words)
SLIDE_LECTURES: dict[int, str] = {
    1: (
        ...
        "Claude Code Workflows with Deterministic Guardrails, Machine-Verifiable Specifications, and Automated Tests. "
        ...
        "Without strict runtime scaffolding, even state-of-the-art foundation models suffer from context window degradation, catastrophic execution loop traps, "
    ...
    3: (
        ...
        "unverified file modifications, and destructive shell invocations."
    ),
    4: (
    ...
    7: (
        ...
        "Python's ast module allows us to construct and analyze the full syntax tree before writing to disk, catching unclosed strings and misaligned blocks."
    ),
    8: (
    ...
    10: (
        ...
        "When the loop detector fires, it returns a structured diagnostic message, allowing the supervisory runtime to take corrective action safely."
    ),
    11: (
    ...
    12: (
        ...
        "Standardizing harness capabilities into agent skills allows engineering teams to version-control, compose, and share guardrails across repositories."
    ),
    13: (
        ...
        "If any single pillar is omitted, the agent's reliability envelope is compromised."
    ),
    14: (
        ...
        "Notice how the five pillars form concentric rings of defense around the agent's central reasoning engine."
    ),
    15: (
    ...
    16: (
        ...
        "This check is mathematically robust against symlink attacks and relative path overrides, guaranteeing absolute containment."
    ),
    17: (
    ...
    19: (
        ...
        "core_harness_stack.py integrates all five pillars into a unified, reusable Python class with zero external dependencies."
    ...
    20: (
        ...
        "Packaging sandbox validation as an agent skill allows developers to invoke validate_sandbox.py across diverse operational workflows."
    ),
    21: (
        ...
        "SDD replaces conversational ambiguity with machine-verifiable markdown specifications, transforming agile user stories into formal contracts."
    ),
    22: (
        ...
        "Prompts invite interpretation; specifications demand compliance."
    ),
    23: (
        ...
        "Every section in SPEC.md serves a vital purpose: objectives align context, scopes prevent sprawl, non-goals block anti-patterns, and criteria guarantee quality."
    ),
    24: (
        ...
        "Explicit non-goals prevent over-engineering simple features into monolithic rewrites."
    ),
    25: (
        ...
        "Executable acceptance criteria bridge the gap between requirements and automated verification."
    ),
    26: (
        ...
        "This creates a predictable rhythm: specify, constrain, verify, and test."
    ),
    27: (
        ...
        "Automated spec verification catches scope violations in milliseconds, before code reaches disk."
    ),
    28: (
    ...
    31: (
        ...
        "Layered defense ensures that even if prompt context is compromised, lower-level hook and OS constraints prevent unauthorized actions."
    ),
    32: (
        ...
        "The PascalCase hook interface provides a standardized, industry-wide protocol for deterministic tool governance."
    ),
    33: (
        ...
        "Static AST analysis and entropy-based secret detection provide immediate feedback, keeping credentials secure."
    ),
    34: (
        ...
        "An agent confined to its workspace can never damage host infrastructure or access sensitive parent files."
    ),
    35: (
    ...
    36: (
        ...
        "Evaluation harnesses ensure that enterprise guardrails remain robust against evolving prompt injection vectors."
    ),
    37: (
        ...
        "The safety checklist serves as an immutable standard for production readiness before deploying agents into repositories."
    ),
    38: (
    ...
    41: (
        ...
        "The key insight is that human approval should be requested selectively, preserving developer flow."
    ),
    42: (
        ...
        " Looking closely at the escalation gateway, the PermissionEscalationGateway class assigns risk tiers to operations. Tools like read_file and list_dir are classified as LOW and auto-approved. write_file is classified as MEDIUM, triggering an event log before approval. Critical operations like git_push are classified as CRITICAL. The gateway checks output/approvals.json for a valid cryptographic request_id token. If absent, the push is blocked; once granted, the gateway writes to pending_push.json without executing destructive pushes."
    ),
    43: (
    ...
    44: (
        ...
        "Tests provide the deterministic ground truth that allows autonomous agents to perceive errors, diagnose root causes, and verify repairs."
    ),
    45: (
        ...
        "The Red-Repair-Green TDA loop mirrors the disciplined practices of senior human software engineers."
    ),
    46: (
    ...
    47: (
        ...
        "Automated traceback injection provides high-density signal directly to the model's reasoning engine."
    ),
    48: (
        ...
        "Anti-regression persistence turns every fixed bug into an automated safeguard against future regressions."
    ),
    49: (
    ...
    54: (
        ...
        "Standardized directory layouts ensure that skills remain discoverable, maintainable, and interoperable across platforms."
    ),
    55: (
    ...
    57: (
        ...
        "Stdio transports offer zero-latency local tool execution, while Streamable HTTP enables scalable cloud integrations."
    ),
    58: (
    ...
    67: (
        ...
        "Git worktree isolation guarantees that unverified code cannot corrupt the main repository working tree."
    ),
    68: (
    ...
    70: (
        ...
        "multi_agent_team_simulator.py demonstrates real Git worktree creation, multi-role handoffs, and clean teardown."
    ...
    86: (
        ...
        "We look forward to seeing the resilient, production-grade agent harnesses you build in your own engineering organizations."
    ),
}

# Rich, distinct technical deep-dives for each slide
SLIDE_DEEP_DIVES: dict[int, str] = {
    1: (
        " As we progress through each module of this course, you will learn how to build production-grade supervisors "
        "that execute in pure standard Python with zero external latency. You will master the art of writing machine-checkable specifications, "
        "configuring Claude Code pre-tool interceptors, creating ephemeral Git worktrees for branch isolation, and auditing repositories with automated readiness gates."
    ),
    2: (
        " Every code snippet and architectural pattern presented in this masterclass has been verified against our live testing suites. "
        "Whether you are working with frontier models like Claude 3.7 Sonnet or local open-weights models served through vLLM, "
        "the harness principles we establish provide universal reliability and security."
    ),
    3: (
        " When an unconstrained agent interacts with a codebase, subtle hallucinations can rapidly corrupt project state. "
        "Without an external harness monitoring execution loops and sandboxing file writes, a single mispredicted token can result in catastrophic data loss. "
        "Understanding these failure modes is the first step toward engineering effective defenses."
    ),
    4: (
        " The core limitation of prompt engineering is that it relies on probabilistic adherence. "
        "When context windows fill up or unexpected compiler errors occur, soft prompt instructions are frequently deprioritized. "
        "Harness engineering replaces probabilistic hope with deterministic software guarantees."
    ),
    5: (
        " In our implementation of ContextTokenBudgeter, we dynamically monitor token allocations across memory files, feature specifications, "
        "active workspace files, and execution outputs. When compiler dumps threaten to saturate the context window, "
        "our head-tail compaction algorithm preserves high-density diagnostic signal while keeping token expenditure strictly controlled."
    ),
    6: (
        " Runaway execution loops represent one of the highest cost risks in autonomous agent deployment. "
        "Our loop detector calculates cryptographic hashes of tool invocation signatures, maintaining a sliding history window. "
        "When duplicate tool invocations are detected without state progression, the harness trips a circuit breaker, halting execution automatically."
    ),
    7: (
        " The Abstract Syntax Tree parsing gate in our harness ensures that broken code never reaches disk. "
        "By compiling proposed file edits in memory using ast.parse, the harness intercepts syntax errors, unclosed strings, and broken indentation, "
        "returning actionable diagnostic feedback to the model while maintaining workspace integrity."
    ),
    8: (
        " Destructive shell commands represent an unacceptable enterprise risk. "
        "Our pre-execution command interceptor utilizes pre-compiled regular expressions to block dangerous shell patterns like recursive deletions, "
        "privilege escalations, and bypass flags before subprocess creation ever occurs."
    ),
    9: (
        " In harness_vs_model_demo.py, we create two parallel sandbox environments: one un-harnessed and one harnessed. "
        "The un-harnessed run enters a repetitive execution loop and deletes the application log directory. "
        "The harnessed run intercepts the loop on attempt two and blocks the deletion command, ensuring that all log files survive completely intact."
    ),
    10: (
        " The terminal execution trace on Slide 10 demonstrates the contrast between unconstrained and harnessed execution. "
        "When the loop detector trips, it returns a structured non-zero exit code with diagnostic context, "
        "allowing the supervisory runtime to intervene cleanly."
    ),
    11: (
        " Examining lines 134 to 154 of harness_vs_model_demo.py highlights the elegance of pure standard-library Python. "
        "By utilizing the re, hashlib, and collections modules, we implement robust loop detection and command sanitization "
        "with zero third-party dependencies."
    ),
    12: (
        " The harness-interception-loop-detector skill manifest in .claude/skills/ encapsulates these defensive capabilities "
        "into a reusable, discoverable agent skill with standardized YAML frontmatter."
    ),
    13: (
        " The five pillars of the Core Harness Stack work together to create an airtight execution envelope around the agent. "
        "Memory files establish persistent rules, scoped tools restrict capabilities, deterministic hooks validate actions, "
        "token budgeters optimize context, and JSONL event streams provide total transparency."
    ),
    14: (
        " Each pillar in this architecture addresses a distinct vulnerability in autonomous agent execution, "
        "ensuring defense-in-depth across the entire software development lifecycle."
    ),
    15: (
        " Concise memory files in CLAUDE.md and AGENTS.md prevent context bloat. "
        "We recommend structuring instructions into four clear sections: Architecture, Commands, Style, and Non-Negotiables, "
        "keeping the total size under 500 lines."
    ),
    16: (
        " Using Path.resolve().is_relative_to(workspace) provides mathematically sound filesystem sandboxing, "
        "resolving symbolic links and relative path overrides to guarantee complete containment within the project directory."
    ),
    17: (
        " Synchronous pre-tool and post-tool middleware hooks allow engineering teams to enforce security and quality standards "
        "in real time before and after tool execution."
    ),
    18: (
        " The 20/20/50/10 token budget allocation ensures that no single component starves the model's reasoning capacity, "
        "while append-only JSONL logging provides complete forensic auditability."
    ),
    19: (
        " In core_harness_stack.py, we see all five pillars implemented cleanly in standard Python, "
        "combining token budgeting, tool permission validation, path sandboxing, and structured event logging."
    ),
    20: (
        " The harness-core-stack-sandbox skill allows developers to programmatically validate repository compliance "
        "with least-privilege tool scoping and filesystem containment rules."
    ),
    21: (
        " Spec-Driven Development transforms ambiguous conversational prompts into formal, machine-executable contracts "
        "that define the exact boundaries of every feature."
    ),
    22: (
        " Natural language prompts invite interpretation and scope creep. "
        "A formal specification removes guesswork by explicitly defining allowed files, forbidden non-goals, and automated test criteria."
    ),
    23: (
        " The four mandatory sections of SPEC.md—Objective, Scope, Non-Goals, and Acceptance Criteria—provide "
        "both human-readable documentation and machine-checkable contracts."
    ),
    24: (
        " Explicit non-goals prevent autonomous agents from over-engineering simple tasks or introducing unrequested dependencies, "
        "keeping the codebase clean and modular."
    ),
    25: (
        " By formulating acceptance criteria as discrete, machine-testable items (AC-01 through AC-04), "
        "we create an objective, automated definition of done."
    ),
    26: (
        " The SDD lifecycle establishes a structured four-phase development rhythm: "
        "ingest specification, generate constrained plan, audit diffs against non-goals, and verify with automated tests."
    ),
    27: (
        " Real-time spec verification intercepts out-of-scope modifications before they touch disk, "
        "preventing silent regressions in adjacent subsystems."
    ),
    28: (
        " In spec_driven_verifier.py, regular expressions parse SPEC.md markdown headings, "
        "normalizing paths against the allowed scope and compiling code diffs with ast.parse before saving to disk."
    ),
    29: (
        " The harness-spec-driven-development skill equips agents with automated tools to self-audit proposed diffs "
        "against project contracts prior to committing code."
    ),
    30: (
        " Enterprise guardrails provide proactive, deterministic protection, "
        "ensuring that autonomous agents operate safely without sacrificing development velocity."
    ),
    31: (
        " The 4-Layer Control Model provides layered defense: system prompts establish intent, tool schemas validate types, "
        "deterministic hooks enforce policy, and OS sandboxing isolates execution."
    ),
    32: (
        " Claude Code's PascalCase hook interface establishes a clean JSON-RPC contract over standard input and output, "
        "allowing hooks to return allow, deny, ask, or defer decisions dynamically."
    ),
    33: (
        " Post-action AST syntax compilation and entropy-based secret scanning provide immediate quality and security gates "
        "on every file modification."
    ),
    34: (
        " Canonical path sandboxing mathematically proves that every file operation remains strictly confined "
        "within the authorized project workspace."
    ),
    35: (
        " Risk-tiered permission modes ensure that safe reads run fast while high-impact actions like git push "
        "require explicit operator authorization."
    ),
    36: (
        " Quantitative evaluation benchmarks allow engineering teams to measure guardrail accuracy and latency scientifically "
        "against adversarial prompt injections."
    ),
    37: (
        " The enterprise safety checklist provides three non-negotiable standards for production readiness: "
        "workspace sandboxing, shell sanitization, and AST secret scanning."
    ),
    38: (
        " In guardrails_engine.py, lines 6 to 26 implement the Claude Code PreToolUse interceptor and PostToolUse AST scanner "
        "using clean standard Python."
    ),
    39: (
        " The harness-guardrails-and-hooks skill standardizes these controls across all repository tasks, "
    ...
    42: (
        " In permission_escalation_gateway.py, critical actions like git push are blocked until an authorized signed entry "
        "is recorded in approvals.json."
    ),
    43: (
        " The harness-permission-escalation-gateway skill seamlessly integrates human-in-the-loop governance "
        "into the agent's tool execution pipeline."
    ),
    44: (
        " Automated tests provide the deterministic ground truth that allows autonomous coding agents to perceive errors, "
        "diagnose failures, and verify fixes."
    ),
    45: (
        " The Red-Repair-Green TDA loop mirrors senior engineering discipline: "
        "prove the failure, capture the traceback, apply the repair, and verify resolution."
    ),
    46: (
    ...
    47: (
        " Programmatically capturing compiler stderr streams eliminates manual copy-paste overhead, "
        "feeding high-density diagnostic signal directly into the model's repair prompt."
    ),
    48: (
        " Anti-regression test persistence automatically turns every fixed bug into a permanent test case, "
        "continuously strengthening repository test coverage."
    ),
    49: (
    ...
    50: (
        " In tda_reliability_pipeline.py, real pytest subprocess executions drive automated agent self-healing "
        "and regression safeguard persistence."
    ),
    51: (
    ...
    60: (
        " In Module 7, mcp_server_demo.py and mcp_client_runner.py demonstrate a complete, live stdio MCP session "
        "with database tool execution and live LLM response synthesis."
    ),
    61: (
...
async def generate_masterclass():
    full_narrations = {num: get_full_narration(num) for num in range(1, 87)}
    ...
    print(f"\n[DONE] Synthesized all 86 slide audio tracks in {elapsed:.1f}s.")
    print(f"Total Combined Duration: {minutes:.2f} minutes ({total_duration:.1f} seconds)")

    # Concatenate into single master MP3 using ffmpeg
    ...
    print(f"\n[*] Concatenating segments into unified Masterclass MP3: {master_1hr_audio}...")
    subprocess.run(
        [
    ...
    # Clean up individual slide tracks to leave ONLY the single master mp3 in audio/
    ...
    if master_1hr_audio.is_file():
        ...
        print(f"\n========================================================================")
        print(f">>> MASTERCLASS AUDIO CREATED SUCCESSFULLY (SINGLE FILE) <<<")
...
if __name__ == "__main__":
    asyncio.run(generate_masterclass())
